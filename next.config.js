...
const config = {
  ...
  // 移除 i18n 配置，因为已经在 middleware.ts 中处理了国际化路由
  async headers() {
    return [
      {
        source: '/(.*)',
        headers: [
          ...
          },
          {
            ...
            value: "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval' https://fonts.googleapis.com https://cdn.jsdelivr.net; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; img-src 'self' data: https:; connect-src 'self' https://safebrowsing.googleapis.com; frame-src 'none'; object-src 'none';"
          },
          {
            key: 'Referrer-Policy',
